...
public class DAO {
    ...
    private static final String ADDACCOUNT="INSERT INTO tbl_User (userId, password, fullName, role) VALUES (?, ?, ?, 2);";

    public boolean insertNewAccount(User account) throws ClassNotFoundException, SQLException {
        boolean check = false;

        try {
            Connection conn = DBUtils.getConnection();
            String sql= ADDACCOUNT;
            PreparedStatement p = conn.prepareStatement(sql);
            p.setString(1, account.getUserId());
            p.setInt(2, account.getPassword());
            p.setString(3, account.getFullName());
